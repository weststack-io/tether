...
//
// Structurally identical to sec.ts; HTTP transport is delegated to
// `@/lib/ingestion/fetcher` (INGEST-005).
...
export async function fetchLatest(): Promise<RawRegulatoryItem[]> {
  const result = await fetchUrl(FINRA_RSS_URL, {
    timeoutMs: FINRA_FETCH_TIMEOUT_MS,
  });
  if (result.ok) {
    const items = parseFinraRss(result.body)
      .map(rssItemToRaw)
    ...
    console.warn(
      `[finra.fetchLatest] live fetch ${FINRA_RSS_URL} failed (${result.error}); falling back to cache`,
    );
  }
  try {
