...
//
// Structurally identical to sec.ts and finra.ts; HTTP transport is
// delegated to `@/lib/ingestion/fetcher` (INGEST-005).
...
export async function fetchLatest(): Promise<RawRegulatoryItem[]> {
  const result = await fetchUrl(CFPB_RSS_URL, {
    timeoutMs: CFPB_FETCH_TIMEOUT_MS,
  });
  if (result.ok) {
    const items = parseCfpbRss(result.body)
      .map(rssItemToRaw)
    ...
    console.warn(
      `[cfpb.fetchLatest] live fetch ${CFPB_RSS_URL} failed (${result.error}); falling back to cache`,
    );
  }
  try {
