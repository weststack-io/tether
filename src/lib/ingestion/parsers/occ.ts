...
//
// Structurally identical to sec.ts, finra.ts, and cfpb.ts -- this is now
// the FOURTH byte-identical copy of the regex/fetch/cache helpers. The
// shared HTTP fetcher utility (INGEST-005) is the right home for these;
// when it lands, all four parsers should be refactored to call into it.
...
const OCC_USER_AGENT = "Tether Compliance Demo dev@tether.local";
...
export async function fetchLatest(): Promise<RawRegulatoryItem[]> {
  try {
    const xml = await fetchRss(OCC_RSS_URL, OCC_FETCH_TIMEOUT_MS);
    ...
    console.warn(
      `[occ.fetchLatest] live fetch ${OCC_RSS_URL} failed (${message}); falling back to cache`,
    );
  }
  try {
